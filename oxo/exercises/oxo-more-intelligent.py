...
Το έξυπνο χαρτί ξεκινά πάντα παίζοντας με τα Χ σε γωνιακό τετράγωνο.
Αν ο παίκτης με τα Ο απαντήσει παίζοντας σ’ ένα πλευρικό τετράγωνο
(θέσεις 1, 3, 5 και 7, σύμφωνα με την αρίθμησή μας) τότε το έξυπνο
χαρτί θα επιλέξει τη γωνία που βρίσκεται διαγωνίως απέναντι από την
αρχική. Αυτή η κίνηση καταλήγει σε ισοπαλία, ενώ υπάρχει καλύτερη 
που οδηγεί με βεβαιότητα τον παίκτη με τα Χ στη νίκη.

Να επεκτείνετε τις οδηγίες του έξυπνου χαρτιού έτσι ώστε να μην 
παρουσιάζουν αυτή την “αδυναμία”. Όταν ο παίκτης με τα Ο ξεκινήσει 
παίζοντας σε πλευρικό τετράγωνο, οι οδηγίες θα πρέπει να επιλέγουν 
την κατάλληλη κίνηση ώστε ο παίκτης με τα Χ να κερδίζει. Στη συνέχεια,
να επεκτείνετε ανάλογα και τη συνάρτηση paperPosition.

Σχόλιο: Η μοναδική αλλαγή σε σχέση με το τελικό πρόγραμμα του 
κεφαλαίου είναι στη 2η κίνηση, στη συνάρτηση paperPosition.
"""
...
def paperPosition(board):
    ...
    if nbMoves == 0:
        ...
        # [τροποποιήθηκε]
        # κίνηση 2η: κέντρο (θέση 4) αν ο Ο έπαιξε σε
        # πλευρικό, αλλιώς κάτω δεξιά γωνία (θέση 8), αν
        # είναι διαθέσιμη, αλλιώς πάνω δεξιά (θέση 2)
        if board.index("O") in (1,3,5,7):
            return 4
        elif board[8] == " ":
            return 8
        else:
        ...
        if position is not None:
            return position
        # αλλιώς σε ελεύθερη γωνία που να δημιουργεί παγίδα
        if board[1] == " ":
            return 2
        else:
            return 6
