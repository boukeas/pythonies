...
Η check να ελέγχει μόνο τριάδες στις οποίες συμμετέχει 
η θέση στην οποία μόλις έπαιξε ο παίκτης. 
...
def check(player, board, position):
    """ Ελέγχει αν υπάρχει 3άδα όπου ο παίκτης player
        έχει καταλάβει και τις 3 θέσεις.
        player: σύμβολο παίκτη ("Χ" ή "Ο")
        board: Μια λίστα με 9 στοιχεία (η τρίλιζα)    
    """
    # για κάθε 3άδα θέσεων στις οποίες συμμετέχει η position
    for triple in partof[position]:
...
# [προστέθηκε] για κάθε θέση, οι τριάδες στις οποίες συμμετέχει αυτή η θέση
partof = tuple(tuple(triple for triple in triples if position in triple) for position in range(9))
...
while blank > 0 and not inarow:
    ...
    blank -= 1
    # [τροποποιήθηκε] έλεγχος για τρίλιζα
    inarow = check(player, board, position)
    # εναλλαγή παίκτη
    player = next(player)
