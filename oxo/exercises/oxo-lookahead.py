...
Όταν φτάσαμε στο σημείο όπου το πρόγραμμά μας έπρεπε να αναλάβει
το ρόλο ενός εκ των δύο παικτών, ήταν απαραίτητο να διαθέτουμε 
έναν μηχανισμό ο οποίος θα επέτρεπε στο πρόγραμμα να ελέγχει αν 
ένας παίκτης έχει τη δυνατότητα να κάνει τρίλιζα. Για τον σκοπό 
αυτό, αναπτύξαμε τη συνάρτηση checkPartial.

Στο βιβλίο του Invent your own computer games with Python, ο Al 
Sweigart αφιερώνει επίσης ένα κεφάλαιο στην τρίλιζα και προτείνει 
τον εξής τρόπο για να ελέγχεται αν ένας παίκτης έχει τη δυνατότητα 
να κάνει τρίλιζα: για κάθε διαθέσιμη θέση δημιουργείται ένα
αντίγραφο του πίνακα του παιχνιδιού, συμπληρώνεται η θέση με το 
σύμβολο του παίκτη και ελέγχεται αν έχει γίνει τρίλιζα. Ουσιαστικά, 
το πρόγραμμα δοκιμάζει τι θα συμβεί στην επόμενη κίνηση.

Να αναπτύξετε μια νέα υλοποίηση της checkPartial που να χρησιμοποιεί 
τη μέθοδο που περιγράψαμε για να ελέγξει αν ένας παίκτης έχει τη 
δυνατότητα να κάνει τρίλιζα. Για να ελέγχει η συνάρτησή σας αν μια 
κίνηση έχει οδηγήσει σε τρίλιζα, θα πρέπει να καλεί τη συνάρτηση check.
"""
...
def checkPartial(player, board):
    """ Ελέγχει αν υπάρχει 3άδα όπου ο παίκτης player
        ...
        board: Μια λίστα με 9 στοιχεία (η τρίλιζα)    
    """
    # για κάθε μία από τις 9 θέσεις
    for position in range(9):
        # αν η θέση είναι διαθέσιμη
        if board[position] == " ":
            # αντίγραφο του πίνακα
            newBoard = board.copy()
            # ο player παίζει στη θέση
